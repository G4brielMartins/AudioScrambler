...
def Sinal(pdf: FPDF, path: str|Path):
    ...
    link_text(pdf)
    pdf.cell(text="Áudio Original", link=f"file://{path / 'original.wav'}", align='C')
    pdf.set_x(pdf.x + 2.2)
    pdf.cell(text="Áudio Embaralhado", link=f"file://{path / 'scrambled.wav'}", align='C')
    pdf.set_x(pdf.x + 2.2)
    pdf.cell(text="Áudio Desembaralhado", link=f"file://{path / 'descrambled.wav'}", align='C')
    base_text(pdf)